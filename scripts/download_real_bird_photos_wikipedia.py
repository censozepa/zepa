...
def get_real_bird_image_url(sci_name):
    parts = sci_name.split()
    binomial = f"{parts[0]} {parts[1]}" if len(parts) >= 2 else sci_name

    apis = [
        ('es.wikipedia.org', binomial),
        ('en.wikipedia.org', binomial),
        ('commons.wikimedia.org', binomial)
    ]

    for domain, query in apis:
        ...
        headers = {'User-Agent': 'CensoZepaOrnithologyApp/2.0'}
        try:
    ...
    try:
        ...
        resp = requests.get(search_url, params=search_params, headers={'User-Agent': 'CensoZepaApp/2.0'}, timeout=6)
        data = resp.json()
        pages = data.get('query', {}).get('pages', {})
        for page_id, page_info in pages.items():
...
def main():
    ...
    print(f"Downloading real bird photographs for {len(species)} species...")
    ...
    for i, (code, sci_name, common_name) in enumerate(species, 1):
        ...
        img_url = get_real_bird_image_url(sci_name)
        success = False
        if img_url:
            try:
